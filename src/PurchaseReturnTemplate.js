...
const Template = () => {
    ...
    useEffect(() => {
        const header = sessionStorage.getItem('PRheaderData');
        const detail = sessionStorage.getItem('PRdetailData');
        const tax = sessionStorage.getItem('PRtaxData');
    ...
    if (!headerData || !detailData || !taxData) {
      ...
      const totalAmount = headerData[0].purchase_amount_returne;
      const totalAmountInWords = `${toWords.convert(totalAmount)} rupees only`;
      
      const total = parseFloat(headerData[0].purchase_amount_returne).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
      const tax = parseFloat(taxData[0].tax_amt).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
      const purchase = parseFloat(headerData[0].total_amount).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    ...
    return (
        <>
            <div className="invoice-container" ref={componentRef}>
                ...
                <h1 className="invoice-title">Purchase Return</h1>
                <div className="invoice-info">
                    <div className="bill-to">
                        <tr>
                            <td><p>Party Code :</p></td>
                            <td><p>{headerData[0].vendor_code}</p></td>
                        </tr>
                        <tr>
                            <td><p>Party Name :</p></td>
                            <td><p>{headerData[0].vendor_name}</p></td>
                        </tr>
                        <tr>
                            <td><p>Return Date :</p></td>
                            <td><p>{new Date(headerData[0].return_date).toLocaleDateString()}</p></td>
                        </tr>
                        <tr>
                            <td><p>Return Reason :</p></td>
                            <td><p>{headerData[0].return_reason}</p></td>
                        </tr>
                    </div>
                ...
                <div className="invoice-summary">
                    ...
                    <p>Terms and Conditions: Thanks for doing business with us!</p>
                </div>
