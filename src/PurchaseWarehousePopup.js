...
export default function PurchaseWarehousePopup({ open, handleClose, handleWarehouse }) {
  ...
  const handlewarehouseSearch = async () => {
    ...
    try {
      const response = await fetch(`${config.apiBaseUrl}/warehouseSearchdata`, {
        ...
        body: JSON.stringify({ warehouse_code, warehouse_name, status, location_no, company_code: sessionStorage.getItem('selectedCompanyCode') }) // Send company_no and company_name as search criteria
      });
      if (response.ok) {
