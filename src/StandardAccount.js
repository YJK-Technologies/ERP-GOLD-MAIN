...
const StandardAcc = () => {
  ...
  const handleSearch = async () => {
    try {
      const response = await fetch(
        `${config.apiBaseUrl}/getstandardsearchdata`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const onCellValueChanged = (params) => {
    ...
    const rowIndex = updatedRowData.findIndex(
      (row) =>
        row.standard_accgroup_code === params.data.standard_accgroup_code &&
        row.standard_accgroup_name === params.data.standard_accgroup_name, // Use the unique identifier
    );
    if (rowIndex !== -1) {
